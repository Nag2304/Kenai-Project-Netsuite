 ...
 define(["N/search", "N/currentRecord", 'N/url', 'N/https'], function (
    ...
    function pageInit(context) {
        try {
            ...
            document.getElementById("submitter").onclick = function () { printOrders() }
            document.getElementById("secondarysubmitter").type = "button"
            document.getElementById("secondarysubmitter").onclick = function () { printOrders() }
    ...
    function printOrders() {
        ...
        for (var i = 0; i < sublistLength; i++) {
            var isSelected = record.getSublistValue("custpage_sublist", "custpage_sublist_print", i)
            if (isSelected) {
                records.push(record.getSublistValue("custpage_sublist", "custpage_sublist_soid", i))
            }
        }
        ...
        if (records.length > 0) {
            var link = urlModule.resolveScript({
                ...
                params: { summary: "T", formType: formType, records: records.toString() }
            })
            ...
            link = urlModule.resolveScript({
                ...
                deploymentId: "customdeploy1",
                returnExternalUrl: true,
                params: { summary: "F", formType: formType, records: records.toString() }
            })
             

            win = window.open(link, '_blank');
            https.get.promise(urlModule.resolveScript({
                scriptId: "customscript_print_orders_ticket",
                deploymentId: "customdeploy1",
                returnExternalUrl: true,
                params: { summary: "F", formType: formType, records: records.toString(), markorders: "T" }
            }))
    ...
    .catch(function onRejected(reason) {
        log.debug({
            ...
            document.location = urlModule.resolveScript({
            ...
            }) + "&item=&reprinting=false&holdship=false&giftNote=false&stack=false&personalized=true&batch=false&ship=&channel=" + record.getValue("custpage_channel") + "&singleline=false"
            
            win.focus();
        }
        else { alert("Please select orders to print") }
    }
    ...
    function applyFilters() {
        ...
        parameterIds.forEach(function (parameterId) {
            ...
            if (locationStr.indexOf("&" + parameterId) > -1) {
                ...
                locationStr = replaceParameter(locationStr, parameterId, fields[parameterId])
            }
    ...
    function replaceParameter(urlStr, parameter, value) {
        ...
        while (endParam > -1 && urlStr[endParam] != "&" && endParam < urlStr.length) endParam++;
