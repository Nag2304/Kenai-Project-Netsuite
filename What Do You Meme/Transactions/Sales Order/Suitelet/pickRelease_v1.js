...
define([
  ...
  function onRequest(context) {
    try {
      ...
      /*else {
         var doc = context.request
         var sublistLength = doc.getLineCount("custpage_sublist")
         var records = []
         for (var i = 0; i < sublistLength; i++) {
           var isSelected = doc.getSublistValue("custpage_sublist", "custpage_sublist_print", i)
           if (isSelected == "T") {
             log.debug('so', doc.getSublistValue("custpage_sublist", "custpage_sublist_soid", i))
             var saleOrder = record.load({
               type: record.Type.SALES_ORDER,
               id: doc.getSublistValue("custpage_sublist", "custpage_sublist_soid", i)
             })
             records.push(getObject(saleOrder))
             //if(request.parameters["custpage_reprint"] == 'F')
             saleOrder.setValue('custbody_printed_ticket', true)
             saleOrder.save()
           }
         }
         /* ----------------------------- CREATE PDF Start ----------------------------- 
         var htmlTemplateFile = undefined;
         if (context.request.parameters["custpage_formtype"] == "1") {
           htmlTemplateFile = file.load("Templates/Pick Templates/pickECommerce.html");
         } else {
           htmlTemplateFile = file.load("Templates/Pick Templates/pickWhole.html");
         }
         var data = {};
         data.records = records;
 
         log.debug("JSON.parse(records[0])", JSON.parse(JSON.stringify(records[0])))
         //data.records.push(JSON.parse(JSON.stringify(records[0])))
 
         log.debug("data.records", data.records)
         dat = JSON.stringify(data)
         log.debug("dat", dat)
         log.debug("dat.records", dat.records)
         log.debug("dat.test", dat.test)
