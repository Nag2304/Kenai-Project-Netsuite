...
define([
  ...
  'N/file',
  'N/runtime',
], function (serverWidget, log, search, record, render, file, runtime) {
  const exports = {};
  /* ---------------------------- On Request Begin ---------------------------- */
  function onRequest(context) {
    try {
      ...
      if (context.request.method === 'GET') {
        ...
        sourceNameOption.addSelectOption({ value: 5, text: 'Amazon' });
  ...
  function checkParameters(request, form) {
    ...
    if (hasFilters == 'T') {
      ...
      /*
       log.debug('checkParameters', 'filters["custpage_from_date"]: ' + filters["custpage_from_date"]);
       log.debug('checkParameters', 'filters["custpage_to_date"]: ' + filters["custpage_to_date"]);
       */
      if (
      ...
      /*
      log.debug('checkParameters', 'filters["custpage_source_name"]: ' + filters["custpage_source_name"]);
      log.debug('checkParameters', '(filters["custpage_source_name"] > 0): ' + (filters["custpage_source_name"] > 0));
*/
      ...
      //       log.debug("checkParameters = parameters", filters)
      //       log.debug("checkParameters = filters", searchFilters)
      loadOrders(
        searchFilters,
        form,
  ...
  function loadOrders(filterArr, form, singleLine, itemName, batch_size) {
    ...
    try {
      ...
      salesorderSearchObj.run().each(function (result) {
        ...
        var orderId = result.getValue(columns[8]);
        ...
        if (
          ((singleLine == 'T' &&
            orderIsSingleLine(result.getValue(columns[internalId_Index]))) ||
            singleLine == 'F') &&
          specificItem
        ) {
          var recordToDisplay = {};
          recordsToDisplay[recordsToDisplay.length] = recordToDisplay;

          var indexPos = 0;
          recordToDisplay.custpage_sublist_date = result.getValue(
            columns[indexPos++]
          );
          recordToDisplay.custpage_sublist_type = result.getValue(
            columns[indexPos++]
          );
          recordToDisplay.custpage_sublist_sonumber = result.getValue(
            columns[indexPos++]
          );
          recordToDisplay.custpage_bin_reference = getBinReference(
            result.getValue(columns[internalId_Index])
          );
          recordToDisplay.custpage_shopify_number = result.getValue(
            columns[indexPos++]
          );
          recordToDisplay.custpage_sublist_customer = result.getValue(
            columns[indexPos++]
          );
          recordToDisplay.custpage_sublist_shipto = result.getValue(
            columns[indexPos++]
          );
          recordToDisplay.custpage_sublist_shipvia = result.getText(
            columns[indexPos++]
          );
          recordToDisplay.custpage_sublist_source_name = result.getText(
            columns[indexPos++]
          );
          recordToDisplay.custpage_sublist_soid = orderId;

          current++;
        }
        var scriptObj = runtime.getCurrentScript();
        var remainingUsage = scriptObj.getRemainingUsage();
        return current < batch_size && 15 < remainingUsage;
      });

      log.debug('Sorting --- recordsToDisplay', recordsToDisplay);
      var recordsToDisplaySorted = [];
      for (var index = 0; index < recordsToDisplay.length; index++) {
        var currentBin = recordsToDisplay[index].custpage_bin_reference;
        if (currentBin.length > 4) {
          currentBin = currentBin.substring(0, 5);
        }
        log.debug('current', currentBin);
        added = false;
        for (var s = 0; s < recordsToDisplaySorted.length; s++) {
          var custpage_bin_reference =
            recordsToDisplaySorted[s].custpage_bin_reference;
          if (custpage_bin_reference.length > 4) {
            custpage_bin_reference.substring(0, 5);
          }
          if (currentBin < custpage_bin_reference) {
            added = true;
            var newArr = [];
            for (var t = 0; t < s; t++) {
              newArr[newArr.length] = recordsToDisplaySorted[t];
            }

            newArr[newArr.length] = recordsToDisplay[index];

            for (var s1 = s; s1 < recordsToDisplaySorted.length; s1++) {
              newArr[newArr.length] = recordsToDisplaySorted[s1];
            }
            recordsToDisplaySorted = newArr;
            break;
          }
        }
        if (!added) {
          recordsToDisplaySorted[recordsToDisplaySorted.length] =
            recordsToDisplay[index];
        }
      }

      log.debug('Sorting --- recordsToDisplaySorted', recordsToDisplaySorted);
      recordsToDisplay = recordsToDisplaySorted;

      for (var index = 0; index < recordsToDisplay.length; index++) {
        var recordToDisplay = recordsToDisplay[index];

        sublist.setSublistValue({
          id: 'custpage_sublist_date',
          line: index,
          value: recordToDisplay.custpage_sublist_date,
        });
        sublist.setSublistValue({
          id: 'custpage_sublist_type',
          line: index,
          value: recordToDisplay.custpage_sublist_type,
        });
        sublist.setSublistValue({
          id: 'custpage_sublist_sonumber',
          line: index,
          value: recordToDisplay.custpage_sublist_sonumber,
        });
        try {
          sublist.setSublistValue({
            id: 'custpage_bin_reference',
            line: index,
            value: recordToDisplay.custpage_bin_reference,
          });
        } catch (E) {}
        sublist.setSublistValue({
          id: 'custpage_shopify_number',
          line: index,
          value: recordToDisplay.custpage_shopify_number,
        });
        sublist.setSublistValue({
          id: 'custpage_sublist_customer',
          line: index,
          value: recordToDisplay.custpage_sublist_customer,
        });
        sublist.setSublistValue({
          id: 'custpage_sublist_shipto',
          line: index,
          value: recordToDisplay.custpage_sublist_shipto,
        });
        sublist.setSublistValue({
          id: 'custpage_sublist_shipvia',
          line: index,
          value: recordToDisplay.custpage_sublist_shipvia,
        });
        sublist.setSublistValue({
          id: 'custpage_sublist_source_name',
          line: index,
          value: recordToDisplay.custpage_sublist_source_name,
        });
        sublist.setSublistValue({
          id: 'custpage_sublist_soid',
          line: index,
          value: recordToDisplay.custpage_sublist_soid,
        });
      }
    } catch (E) {
      log.error('loadOrders ERROR', E);
    }
  }
  ...
  function getBinReference(orderId) {
    var transactionSearchObj = search.create({
      ...
      columns: [search.createColumn({ name: 'custcol_ld_bin_reference' })],
    });

    var bin_reference = '';
    try {
      transactionSearchObj.run().each(function (result) {
        bin_reference += result.getText('custcol_ld_bin_reference') + ' / ';
        return true;
      });
    } catch (E) {}
    if (bin_reference.length > 1) {
      bin_reference = bin_reference.substring(0, bin_reference.length - 3);
    }
    return bin_reference;
  }
