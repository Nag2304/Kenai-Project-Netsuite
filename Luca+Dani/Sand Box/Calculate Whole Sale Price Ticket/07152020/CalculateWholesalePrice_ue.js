 ...
 */ function (search, serverWidget, log) {
  ...
  function saveWholesalePrice(scriptContext) {
    ...
    /* ----------------- Latest Changes Made on 07/15/2020 Begin ---------------- */
    //* Added if condition to check if customer has value are not.
    //* Added try and catch block to proceed code execution.
    //* Added if condition to check if no lines greater than 0.
    try {
      if (customer) {
        ...
        if (lines > 0) {
          for (var i = 0; i < lines; i++) {
            ...
            if (wholesaleprice_calculated == true) continue;
        ...
        log.debug({ title: "Entity Internal ID is null" });
      }
    } catch (err) {
      log.debug({
        title: "Error Occured While executing script",
        details: err,
      });
    }

    /* ----------------- Latest Changes Made on 07/15/2020 End ---------------- */
  }
