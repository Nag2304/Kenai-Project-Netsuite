 ...
 */ function (ui, search) {
  /* ---------------------- Changes Done Begin 06/24/2020 --------------------- */
  var storageArray = [];
  /* ---------------------- Changes Done End 06/24/2020 --------------------- */
  ...
  function fieldChanged(scriptContext) {
    ...
    if (sublist_id == "item" && fieldid == "item") {
      ...
      /* ------------------ Changes Made on the 07/15/2020 Begin ------------------ */
      //* Added if condition to check if customer exists or not
      //* Added try and catch block to prevent exceptions.
      try {
        if (customer) {
          ...
          var wholesale_discount =
            customer_fieldLookUp.custentity_discount_offered;
          ...
          log.debug({ title: "Customer field value is missing." });
        }
      } catch (err) {
        log.debug({ title: "field changed event", details: err });
      }
      /* ------------------ Changes Made on the 07/15/2020 End ------------------ */
    }
  }
  ...
  function validateLine(scriptContext) {
    ...
    /* ---------------------- Changes Done 06/24/2020  Begin  --------------------- */
    log.debug({ title: "Before Whole Sale Price", details: wholesale_price });
    log.debug({ title: "Whole Sale Discount", details: wholesale_discount });
    ...
    log.debug({ title: "Storage Array", details: storageArray });
    log.debug({ title: "After Whole Sale Price", details: wholesale_price });

    if (storageArray.length > 0) {
      for (var m = 0; m < storageArray.length; m++) {
        if (
          rec.getCurrentSublistValue({ sublistId: "item", fieldId: "item" }) ===
            storageArray[m].itemId &&
          storageArray[m].itemId !== ""
        ) {
          log.debug({ title: "Item Matched" });
          if (storageArray[m].oldValue !== currentPrice) {
            log.debug({ title: "Price Matched" });
            rec.setCurrentSublistValue({
              sublistId: "item",
              fieldId: "rate",
              value: currentPrice,
