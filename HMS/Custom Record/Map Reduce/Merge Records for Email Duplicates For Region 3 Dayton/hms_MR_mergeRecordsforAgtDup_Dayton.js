...
/**
 * File name: hms_MR_mergeRecordsforAgtDup_Dayton.js
 * Script: HMS | MR Merge Agt Dup Dayton
 * Author           Date       Version               Remarks
 * nagendrababu 31st Oct 2024   1.00        Initial creation of the script.
 *
 */
...
define(['N/search', 'N/record'], (search, record) => {
  ...
  const map = (mapContext) => {
    ...
    try {
      ...
      // Form Key
      const key = results.values.custrecord_hms_agent_id_number;
      //
      ...
      for (let index = 0; index < values.length; index++) {
        if (values.length > 1) {
          ...
          if (
            ...
            originalAgentInternalPurgeObj = retrieveAgentRecordId(key);
            log.debug(
   ...
   */
  const retrieveAgentRecordId = (agentId) => {
    ...
    try {
      const agentOriginalSearch = search.create({
        ...
        filters: [['custrecord_agent_id', 'is', agentId]],
        columns: [
