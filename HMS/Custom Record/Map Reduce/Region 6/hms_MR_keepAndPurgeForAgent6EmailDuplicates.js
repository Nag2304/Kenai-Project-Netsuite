...
/**
 * File name: hms_MR_keepAndPurgeEmailDuplicateRecordsForRegion6.js
 * Script: HMS | MR K & P For Region 6 Email Dup
 * Author           Date       Version               Remarks
 * nagendrababu  02.26.2025     1.00     Initial Creation of the Script.
 *
 */
...
define(['N/search', 'N/record'], (search, record) => {
  ...
  const map = (mapContext) => {
    ...
    try {
      ...
      // Key
      const email =
        mapContextValues.values['GROUP(custrecord_hms_agent_email_6)'];
      //

      // Values
      const reduceValues = {};
      reduceValues.crmCount = parseInt(
        mapContextValues.values['SUM(custrecord_hms_crm_record_count_6)']
      );
      reduceValues.surveyCount = parseInt(
        mapContextValues.values['SUM(custrecord_hms_survery_count_6)']
      );
      reduceValues.soldPropertiesCount = parseInt(
        mapContextValues.values['SUM(custrecord_hms_sold_properties_6)']
      );
      //
  ...
  const reduce = (reduceContext) => {
    ...
    try {
      ...
      const determineKeepPurge = (rec1, rec2) => {
        ...
        // 6. If neither record has related records and neither is verified, keep the latest one
        else {
          return rec1LastUpdate > rec2LastUpdate
            ? { keep: rec1, purge: rec2 }
            : { keep: rec2, purge: rec1 };
  ...
  const retrieveAgentInternalIds = (email) => {
    ...
    try {
      ...
      const customrecord_hms_agent_upd_projectSearchObj = search.create({
        type: 'customrecord_hms_agent_upd_prjct_reg_6',
        filters: [
          ['custrecord_hms_agent_email_6', 'is', email],
          'AND',
          ['custrecord_hms_email_dupe_6', 'is', 'T'],
          'AND',
          ['isinactive', 'is', 'F'],
          'AND',
          ['custrecord_hms_keep_6', 'is', 'F'],
          'AND',
          ['custrecord_hms_purge_6', 'is', 'F'],
        ],
        columns: [
          search.createColumn({
            name: 'custrecord_hms_agent_id_number_6',
            label: 'Agent ID Number',
          }),
          search.createColumn({
            name: 'custrecord_hms_verified_from_ret_feed_6',
            label: 'VERIFIED FROM RETS FEED',
          }),
          search.createColumn({
            name: 'custrecord_hms_crm_record_count_6',
            label: 'CRM Record Count',
          }),
          search.createColumn({
            name: 'custrecord_hms_survery_count_6',
            label: 'Survey Count',
          }),
          search.createColumn({
            name: 'custrecord_hms_sold_properties_6',
            label: 'Sold Properties',
          }),
          search.createColumn({
            name: 'custrecord_hms_first_name_6',
            label: 'First Name',
          }),
          search.createColumn({
            name: 'custrecord_hms_last_name_6',
            label: 'Last Name',
          }),
          search.createColumn({
            name: 'custrecord_hms_last_update_6',
            label: 'Last Update',
          }),
        ],
      });
      ...
      if (searchResultCount == 2) {
        customrecord_hms_agent_upd_projectSearchObj.run().each((result) => {
          ...
          resultObj.agentIdNumber = result.getValue(
            'custrecord_hms_agent_id_number_6'
          );
          resultObj.verifiedFromRETSFeeds = result.getValue(
            'custrecord_hms_verified_from_ret_feed_6'
          );
          resultObj.cCount = result.getValue(
            'custrecord_hms_crm_record_count_6'
          );
          resultObj.sCount = result.getValue('custrecord_hms_survery_count_6');
          resultObj.spCount = result.getValue(
            'custrecord_hms_sold_properties_6'
          );
          resultObj.firstName = result.getValue('custrecord_hms_first_name_6');
          resultObj.lastName = result.getValue('custrecord_hms_last_name_6');
          resultObj.lastUpdate = result.getValue(
            'custrecord_hms_last_update_6'
          );
  ...
  const searchAgentDuplicateswithTwoRecords = () => {
    ...
    return search.create({
      ...
      columns: [
        search.createColumn({
          name: 'custrecord_hms_email_dupe_6',
          summary: 'GROUP',
          label: 'Email ID Duplicate',
        }),
        search.createColumn({
          name: 'custrecord_hms_agent_email_6',
          summary: 'GROUP',
          label: 'EMAIL',
        }),
        search.createColumn({
        ...
        search.createColumn({
          name: 'custrecord_hms_mls_region_6',
          summary: 'GROUP',
          label: 'MLS Region',
        }),
      ],
    ...
    try {
      if (customRecordId && flag == 'purge') {
        record.submitFields({
          ...
          id: customRecordId,
          values: {
            custrecord_hms_purge_6: true,
            custrecord_hms_keep_6: false,
          },
        });
        ...
        record.submitFields({
          ...
          id: customRecordId,
          values: {
            custrecord_hms_keep_6: true,
            custrecord_hms_purge_6: false,
          },
        });
        log.debug(loggerTitle, `Updated ID: ${customRecordId} for ${flag}`);
      }
