...
/**
 * File name: hms_MR_mergeEmail6DuplicateRecords.js
 * Script: HMS | MR Merge Email 6 Duplicate Records
 * Author           Date       Version               Remarks
 * nagendrababu  06.19.2025      1.00     Initial Creation of the Script.
 *
 */
...
define(['N/search', 'N/record', 'N/runtime'], (search, record, runtime) => {
  ...
  const reduce = (reduceContext) => {
    ...
    try {
      ...
      // Read the Values
      const email =
        reduceContextValues.values['GROUP(custrecord_hms_agent_email_6)'];
  ...
  const searchEmailDuplicateswithTwoRecords = () => {
    ...
    if (emailIdParameter) {
      filtersArr = [
        ...
        ['isinactive', 'is', 'F'],
        'AND',
        ['custrecord_hms_agent_email_6', 'is', emailIdParameter],
      ];
    } else {
      filtersArr = [
        ['custrecord_hms_email_dupe_6', 'is', 'T'],
    ...
    try {
      customAgentUpdateProjectSearchObj = search.create({
        ...
        columns: [
          ...
          search.createColumn({
            name: 'custrecord_hms_nrds_6',
            label: 'NRDS',
          }),
          search.createColumn({
            name: 'custrecord_hms_agent_name_6',
            label: 'Name',
          }),
          search.createColumn({
            name: 'custrecord_hms_keep_6',
            label: 'Keep',
            sort: search.Sort.DESC,
          }),
          search.createColumn({
            name: 'custrecord_hms_purge_6',
            label: 'Purge',
          }),
          search.createColumn({
          ...
          search.createColumn({
            name: 'custrecord_hms_brokerage_name_6',
            label: 'brokerage name',
          }),
          search.createColumn({
            name: 'custrecord_hms_orig_agt_rec_6',
      ...
      customAgentUpdateProjectSearchObj.run().each((result) => {
        ...
        const name = result.getValue('custrecord_hms_brokerage_name_6');
        const agentName = removeSpecialCharacters(
          result.getValue('custrecord_hms_agent_name_6')
        );
        log.debug(loggerTitle + ' Before callng the getAgent Record ID', {
          agentIdNumber,
          name,
        });
        const agentId = result.getValue('custrecord_hms_orig_agt_rec_6');
        //
        const keep = result.getValue('custrecord_hms_keep_6');
        const purge = result.getValue('custrecord_hms_purge_6');
        const nrdsId = result.getValue('custrecord_hms_nrds_6');
        const crmCount = result.getValue('custrecord_hms_crm_record_count_6');
