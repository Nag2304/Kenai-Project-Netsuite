...
/**
 * File name: hms_MR_mergeAgent1DuplicateRecords.js
 * Script: HMS | MR Merge Agent 1 Duplicate Record
...
define(['N/search', 'N/record', 'N/runtime'], (search, record, runtime) => {
  ...
  const reduce = (reduceContext) => {
    ...
    try {
      ...
      // Read the Values
      const agentIdNumber =
        reduceContextValues.values['GROUP(custrecord_hms_agent_id_number_1)'];
  ...
  const searchAgentDuplicateswithTwoRecords = () => {
    ...
    if (agentIdParameter) {
      filtersArr = [
        ['custrecord_hms_agent_id_dupe_1', 'is', 'T'],
        'AND',
        ['isinactive', 'is', 'F'],
        'AND',
        ['custrecord_hms_agent_id_number_1', 'is', agentIdParameter],
      ];
    } else {
      filtersArr = [
        ['custrecord_hms_agent_id_dupe_1', 'is', 'T'],
  ...
  const getAgentRelatedRecords = (agentIdNumber) => {
    ...
    try {
      customAgentUpdateProjectSearchObj = search.create({
        type: 'customrecord_hms_agent_upd_prjct_reg_1',
        filters: [
          ['custrecord_hms_agent_id_number_1', 'is', agentIdNumber],
          'AND',
          ['custrecord_hms_agent_id_dupe_1', 'is', 'T'],
          'AND',
          ['isinactive', 'is', 'F'],
