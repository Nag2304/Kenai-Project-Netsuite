...
/**
 * File name: hms_MR_mergeEmail7DuplicateRecords.js
 * Script: HMS | MR Merge Email 7 Duplicate Records
...
define(['N/search', 'N/record', 'N/runtime'], (search, record, runtime) => {
  ...
  const reduce = (reduceContext) => {
    ...
    try {
      ...
      // Read the Values
      const email =
        reduceContextValues.values['GROUP(custrecord_hms_agent_email_7)'];
  ...
  const searchEmailDuplicateswithTwoRecords = () => {
    ...
    if (emailIdParameter) {
      filtersArr = [
        ...
        ['isinactive', 'is', 'F'],
        'AND',
        ['custrecord_hms_agent_email_7', 'is', emailIdParameter],
      ];
    } else {
      filtersArr = [
        ['custrecord_hms_email_dupe_7', 'is', 'T'],
    ...
    try {
      customAgentUpdateProjectSearchObj = search.create({
        ...
        columns: [
          ...
          search.createColumn({
            name: 'custrecord_hms_nrds_7',
            label: 'NRDS',
          }),
          search.createColumn({
            name: 'custrecord_hms_agent_name_7',
            label: 'Name',
          }),
          search.createColumn({
            name: 'custrecord_hms_keep_7',
            label: 'Keep',
            sort: search.Sort.DESC,
          }),
          search.createColumn({
            name: 'custrecord_hms_purge_7',
            label: 'Purge',
          }),
          search.createColumn({
          ...
          search.createColumn({
            name: 'custrecord_hms_brokerage_name_7',
            label: 'brokerage name',
          }),
          search.createColumn({
            name: 'custrecord_hms_orig_agt_rec_7',
      ...
      customAgentUpdateProjectSearchObj.run().each((result) => {
        ...
        const name = result.getValue('custrecord_hms_brokerage_name_7');
        const agentName = removeSpecialCharacters(
          result.getValue('custrecord_hms_agent_name_7')
        );
        log.debug(loggerTitle + ' Before callng the getAgent Record ID', {
          agentIdNumber,
          name,
        });
        const agentId = result.getValue('custrecord_hms_orig_agt_rec_7');
        //
        const keep = result.getValue('custrecord_hms_keep_7');
        const purge = result.getValue('custrecord_hms_purge_7');
        const nrdsId = result.getValue('custrecord_hms_nrds_7');
        const crmCount = result.getValue('custrecord_hms_crm_record_count_7');
