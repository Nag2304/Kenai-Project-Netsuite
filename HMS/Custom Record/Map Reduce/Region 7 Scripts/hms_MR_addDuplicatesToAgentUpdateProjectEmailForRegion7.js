...
/**
 * File name: hms_MR_addDuplicatesToAgentUpdateProjectEmailForRegion7.js
 * Script: HMS | MR Add Email Dup for Region 7
...
define(['N/record', 'N/search'], (record, search) => {
  ...
  const getInputData = () => {
    ...
    return search.create({
      ...
      filters: [
        ['custrecord_agent_mls_region.internalidnumber', 'equalto', '7'],
        'AND',
        ['isinactive', 'is', 'F'],
      ],
      columns: [
        ...
        for (let index = 0; index < values.length; index++) {
          ...
          if (emailFirstNameCounts.get(keyValuePair) > 1) {
            ...
            agentProjectUpdateRecord.setValue({
              fieldId: 'custrecord_hms_agent_id_number_7',
              value: agentName,
            });
            // Extracting name
            agentProjectUpdateRecord.setValue({
              fieldId: 'custrecord_hms_agent_name_7',
              value: name,
            });
            // Extracting MLS region
            agentProjectUpdateRecord.setValue({
              fieldId: 'custrecord_hms_mls_region_7',
              value: mlsRegion,
            });
            // Extracting Email
            agentProjectUpdateRecord.setValue({
              fieldId: 'custrecord_hms_agent_email_7',
              value: key,
            });
            // Extracting agent's first name
            agentProjectUpdateRecord.setValue({
              fieldId: 'custrecord_hms_first_name_7',
              value: agentFirstName,
            });
            // Extracting agent's last name
            agentProjectUpdateRecord.setValue({
              fieldId: 'custrecord_hms_last_name_7',
              value: agentLastName,
            });
            // Extracting brokerage
            agentProjectUpdateRecord.setValue({
              fieldId: 'custrecord_hms_brokerage_name_7',
              value: brokerage,
            });
            // Extracting agent's mobile number
            agentProjectUpdateRecord.setValue({
              fieldId: 'custrecord_hms_cell_phone_7',
              value: agentMobileNumber,
            });
            // Extracting Agent Type
            agentProjectUpdateRecord.setValue({
              fieldId: 'custrecord_hms_agent_type_7',
              value: agentType,
            });
            // Extracting Last Modified
            agentProjectUpdateRecord.setValue({
              fieldId: 'custrecord_hms_last_update_7',
              value: lastmodified,
            });
            // Extracting Prefred Call Back Number
            agentProjectUpdateRecord.setValue({
              fieldId: 'custrecord_hms_pref_callback_7',
              value: callbackNumber,
            });
            // Extracting NRDS ID
            agentProjectUpdateRecord.setValue({
              fieldId: 'custrecord_hms_nrds_7',
              value: nrdsId,
            });
            // Extracting change agent
            agentProjectUpdateRecord.setValue({
              fieldId: 'custrecord_hms_latest_rets_chg_7',
              value: changeAgent,
            });
            // Save the Duplicate Email ID field
            agentProjectUpdateRecord.setValue({
              fieldId: 'custrecord_hms_email_dupe_7',
              value: true,
            });
            // Update the Verified RETS Feed
            if (verifiedFromRETSFeed == 'T' || verifiedFromRETSFeed == true) {
              agentProjectUpdateRecord.setValue({
                fieldId: 'custrecord_hms_verified_from_ret_feed_7',
                value: true,
              });
            } else if (
            ...
            if (agentInternalId) {
              // CRM Record Count
              agentProjectUpdateRecord.setValue({
                fieldId: 'custrecord_hms_crm_record_count_7',
                value: getCRMRecordCount(agentInternalId),
              });
              // Survey Count
              agentProjectUpdateRecord.setValue({
                fieldId: 'custrecord_hms_survery_count_7',
                value: getSurveyCount(agentInternalId),
              });
              // Sold Properties Count
              agentProjectUpdateRecord.setValue({
                fieldId: 'custrecord_hms_sold_properties_7',
                value: getSoldPropertiesCount(agentInternalId),
              });
            }
            agentProjectUpdateRecord.setValue({
              fieldId: 'custrecord_hms_orig_agt_rec_7',
