...
/**
 * File name: hms_MR_addDuplicatesToAgentUpdateProjectEmailForRegion4.js
 * Script: HMS | MR Add Email Dup for Region 4
...
define(['N/record', 'N/search'], (record, search) => {
  ...
  const getInputData = () => {
    ...
    return search.create({
      ...
      filters: [
        ['custrecord_agent_mls_region.internalidnumber', 'equalto', '4'],
        'AND',
        ['isinactive', 'is', 'F'],
      ],
      columns: [
      ...
      if (values.length > 1) {
        ...
        for (let index = 0; index < Math.min(values.length, 2); index++) {
          ...
          agentProjectUpdateRecord.setValue({
            fieldId: 'custrecord_hms_agent_id_number_4',
            value: agentName,
          });
          // Extracting name
          agentProjectUpdateRecord.setValue({
            fieldId: 'custrecord_hms_agent_name_4',
            value: name,
          });
          // Extracting MLS region
          agentProjectUpdateRecord.setValue({
            fieldId: 'custrecord_hms_mls_region_4',
            value: mlsRegion,
          });
          // Extracting Email
          agentProjectUpdateRecord.setValue({
            fieldId: 'custrecord_hms_agent_email_4',
            value: key,
          });
          // Extracting agent's first name
          agentProjectUpdateRecord.setValue({
            fieldId: 'custrecord_hms_first_name_4',
            value: agentFirstName,
          });
          // Extracting agent's last name
          agentProjectUpdateRecord.setValue({
            fieldId: 'custrecord_hms_last_name_4',
            value: agentLastName,
          });
          // Extracting brokerage
          agentProjectUpdateRecord.setValue({
            fieldId: 'custrecord_hms_brokerage_name_4',
            value: brokerage,
          });
          // Extracting agent's mobile number
          agentProjectUpdateRecord.setValue({
            fieldId: 'custrecord_hms_cell_phone_4',
            value: agentMobileNumber,
          });
          // Extracting Agent Type
          agentProjectUpdateRecord.setValue({
            fieldId: 'custrecord_hms_agent_type_4',
            value: agentType,
          });
          // Extracting Last Modified
          agentProjectUpdateRecord.setValue({
            fieldId: 'custrecord_hms_last_update_4',
            value: lastmodified,
          });
          // Extracting Prefred Call Back Number
          agentProjectUpdateRecord.setValue({
            fieldId: 'custrecord_hms_pref_callback_4',
            value: callbackNumber,
          });
          // Extracting NRDS ID
          agentProjectUpdateRecord.setValue({
            fieldId: 'custrecord_hms_nrds_4',
            value: nrdsId,
          });
          // Extracting change agent
          agentProjectUpdateRecord.setValue({
            fieldId: 'custrecord_hms_latest_rets_chg_4',
            value: changeAgent,
          });
          // Save the Duplicate Email ID field
          agentProjectUpdateRecord.setValue({
            fieldId: 'custrecord_hms_email_dupe_4',
            value: true,
          });
          // Update the Verified RETS Feed
          if (verifiedFromRETSFeed == 'T' || verifiedFromRETSFeed == true) {
            agentProjectUpdateRecord.setValue({
              fieldId: 'custrecord_hms_verified_from_ret_feed_4',
              value: true,
            });
          } else if (
          ...
          if (agentInternalId) {
            // CRM Record Count
            agentProjectUpdateRecord.setValue({
              fieldId: 'custrecord_hms_crm_record_count_4',
              value: getCRMRecordCount(agentInternalId),
            });
            // Survey Count
            agentProjectUpdateRecord.setValue({
              fieldId: 'custrecord_hms_survery_count_4',
              value: getSurveyCount(agentInternalId),
            });
            // Sold Properties Count
            agentProjectUpdateRecord.setValue({
              fieldId: 'custrecord_hms_sold_properties_4',
              value: getSoldPropertiesCount(agentInternalId),
            });
          }
          agentProjectUpdateRecord.setValue({
            fieldId: 'custrecord_hms_orig_agt_rec_4',
    ...
    try {
      const customrecord_agentSearchObj = search.create({
        ...
        filters: [
          ['isinactive', 'is', 'F'],
          'AND',
          ['custrecord_agent_mls_region', 'anyof', '4'],
          'AND',
