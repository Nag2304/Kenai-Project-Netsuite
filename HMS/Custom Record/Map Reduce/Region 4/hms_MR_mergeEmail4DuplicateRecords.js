...
/**
 * File name: hms_MR_mergeEmail4DuplicateRecords.js
 * Script: HMS | MR Merge Email 4 Duplicate Records
...
define(['N/search', 'N/record', 'N/runtime'], (search, record, runtime) => {
  ...
  const reduce = (reduceContext) => {
    ...
    try {
      ...
      // Read the Values
      const email =
        reduceContextValues.values['GROUP(custrecord_hms_agent_email_4)'];
  ...
  const searchEmailDuplicateswithTwoRecords = () => {
    ...
    if (emailIdParameter) {
      filtersArr = [
        ['custrecord_hms_email_dupe_4', 'is', 'T'],
        'AND',
        ['isinactive', 'is', 'F'],
        'AND',
        ['custrecord_hms_agent_email_4', 'is', emailIdParameter],
      ];
    } else {
      filtersArr = [
        ['custrecord_hms_email_dupe_4', 'is', 'T'],
    ...
    return search.create({
      ...
      columns: [
        search.createColumn({
          name: 'custrecord_hms_agent_email_4',
          summary: 'GROUP',
          label: 'Email',
        }),
      ],
  ...
  const getAgentRelatedRecords = (email) => {
    ...
    try {
      customAgentUpdateProjectSearchObj = search.create({
        type: 'customrecord_hms_agent_upd_prjct_reg_4',
        filters: [
          ['custrecord_hms_agent_email_4', 'is', email],
          'AND',
          ['custrecord_hms_email_dupe_4', 'is', 'T'],
          'AND',
          ['isinactive', 'is', 'F'],
        ...
        columns: [
          ...
          search.createColumn({
            name: 'custrecord_hms_nrds_4',
            label: 'NRDS',
          }),
          search.createColumn({
            name: 'custrecord_hms_agent_name_4',
            label: 'Name',
          }),
          search.createColumn({
            name: 'custrecord_hms_keep_4',
            label: 'Keep',
            sort: search.Sort.DESC,
          }),
          search.createColumn({
            name: 'custrecord_hms_purge_4',
            label: 'Purge',
          }),
          search.createColumn({
          ...
          search.createColumn({
            name: 'custrecord_hms_brokerage_name_4',
            label: 'brokerage name',
          }),
          search.createColumn({
            name: 'custrecord_hms_orig_agt_rec_4',
      ...
      customAgentUpdateProjectSearchObj.run().each((result) => {
        ...
        const name = result.getValue('custrecord_hms_brokerage_name_4');
        const agentName = removeSpecialCharacters(
          result.getValue('custrecord_hms_agent_name_4')
        );
        log.debug(loggerTitle + ' Before callng the getAgent Record ID', {
          agentIdNumber,
          name,
        });
        const agentId = result.getValue('custrecord_hms_orig_agt_rec_4');
        //
        const keep = result.getValue('custrecord_hms_keep_4');
        const purge = result.getValue('custrecord_hms_purge_4');
        const nrdsId = result.getValue('custrecord_hms_nrds_4');
        const crmCount = result.getValue('custrecord_hms_crm_record_count_4');
