...
/**
 * File name: hms_MR_addDuplicatesToAgentUpdateProjectEmailForRegion17.js
 * Script: HMS | MR Add Email Dup for Region 17
...
define(['N/record', 'N/search'], (record, search) => {
  ...
  const getInputData = () => {
    ...
    return search.create({
      ...
      filters: [
        ['custrecord_agent_mls_region.internalidnumber', 'equalto', '17'],
        'AND',
        ['isinactive', 'is', 'F'],
      ],
      columns: [
      ...
      if (values.length > 1) {
        ...
        for (let index = 0; index < Math.min(values.length, 2); index++) {
          ...
          agentProjectUpdateRecord.setValue({
            fieldId: 'custrecord_hms_agent_id_number_17',
            value: agentName,
          });
          // Extracting name
          agentProjectUpdateRecord.setValue({
            fieldId: 'custrecord_hms_agent_name_17',
            value: name,
          });
          // Extracting MLS region
          agentProjectUpdateRecord.setValue({
            fieldId: 'custrecord_hms_mls_region_17',
            value: mlsRegion,
          });
          // Extracting Email
          agentProjectUpdateRecord.setValue({
            fieldId: 'custrecord_hms_agent_email_17',
            value: key,
          });
          // Extracting agent's first name
          agentProjectUpdateRecord.setValue({
            fieldId: 'custrecord_hms_first_name_17',
            value: agentFirstName,
          });
          // Extracting agent's last name
          agentProjectUpdateRecord.setValue({
            fieldId: 'custrecord_hms_last_name_17',
            value: agentLastName,
          });
          // Extracting brokerage
          agentProjectUpdateRecord.setValue({
            fieldId: 'custrecord_hms_brokerage_name_17',
            value: brokerage,
          });
          // Extracting agent's mobile number
          agentProjectUpdateRecord.setValue({
            fieldId: 'custrecord_hms_cell_phone_17',
            value: agentMobileNumber,
          });
          // Extracting Agent Type
          agentProjectUpdateRecord.setValue({
            fieldId: 'custrecord_hms_agent_type_17',
            value: agentType,
          });
          // Extracting Last Modified
          agentProjectUpdateRecord.setValue({
            fieldId: 'custrecord_hms_last_update_17',
            value: lastmodified,
          });
          // Extracting Prefred Call Back Number
          agentProjectUpdateRecord.setValue({
            fieldId: 'custrecord_hms_pref_callback_17',
            value: callbackNumber,
          });
          // Extracting NRDS ID
          agentProjectUpdateRecord.setValue({
            fieldId: 'custrecord_hms_nrds_17',
            value: nrdsId,
          });
          // Extracting change agent
          agentProjectUpdateRecord.setValue({
            fieldId: 'custrecord_hms_latest_rets_chg_17',
            value: changeAgent,
          });
          // Save the Duplicate Email ID field
          agentProjectUpdateRecord.setValue({
            fieldId: 'custrecord_hms_email_dupe_17',
            value: true,
          });
          // Update the Verified RETS Feed
          if (verifiedFromRETSFeed == 'T' || verifiedFromRETSFeed == true) {
            agentProjectUpdateRecord.setValue({
              fieldId: 'custrecord_hms_verified_from_ret_feed_17',
              value: true,
            });
          } else if (
          ...
          if (agentInternalId) {
            // CRM Record Count
            agentProjectUpdateRecord.setValue({
              fieldId: 'custrecord_hms_crm_record_count_17',
              value: getCRMRecordCount(agentInternalId),
            });
            // Survey Count
            agentProjectUpdateRecord.setValue({
              fieldId: 'custrecord_hms_survery_count_17',
              value: getSurveyCount(agentInternalId),
            });
            // Sold Properties Count
            agentProjectUpdateRecord.setValue({
              fieldId: 'custrecord_hms_sold_properties_17',
              value: getSoldPropertiesCount(agentInternalId),
            });
          }
          agentProjectUpdateRecord.setValue({
            fieldId: 'custrecord_hms_orig_agt_rec_17',
    ...
    try {
      const customrecord_agentSearchObj = search.create({
        ...
        filters: [
          ['isinactive', 'is', 'F'],
          'AND',
          ['custrecord_agent_mls_region', 'anyof', '17'],
          'AND',
