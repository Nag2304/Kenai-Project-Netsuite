...
/**
 * File name: hms_MR_keepAndPurgeEmailDuplicateRecordsForRegion17.js
 * Script: HMS | MR K & P For Region 17 Email Dup
...
define(['N/search', 'N/record'], (search, record) => {
  ...
  const map = (mapContext) => {
    ...
    try {
      ...
      // Key
      const email =
        mapContextValues.values['GROUP(custrecord_hms_agent_email_17)'];
      //

      // Values
      const reduceValues = {};
      reduceValues.crmCount = parseInt(
        mapContextValues.values['SUM(custrecord_hms_crm_record_count_17)']
      );
      reduceValues.surveyCount = parseInt(
        mapContextValues.values['SUM(custrecord_hms_survery_count_17)']
      );
      reduceValues.soldPropertiesCount = parseInt(
        mapContextValues.values['SUM(custrecord_hms_sold_properties_17)']
      );
      //
  ...
  const retrieveAgentInternalIds = (email) => {
    ...
    try {
      ...
      const customrecord_hms_agent_upd_projectSearchObj = search.create({
        type: 'customrecord_hms_agent_upd_prjct_reg_17',
        filters: [
          ['custrecord_hms_agent_email_17', 'is', email],
          'AND',
          ['custrecord_hms_email_dupe_17', 'is', 'T'],
          'AND',
          ['isinactive', 'is', 'F'],
          'AND',
          ['custrecord_hms_keep_17', 'is', 'F'],
          'AND',
          ['custrecord_hms_purge_17', 'is', 'F'],
        ],
        columns: [
          search.createColumn({
            name: 'custrecord_hms_agent_id_number_17',
            label: 'Agent ID Number',
          }),
          search.createColumn({
            name: 'custrecord_hms_verified_from_ret_feed_17',
            label: 'VERIFIED FROM RETS FEED',
          }),
          search.createColumn({
            name: 'custrecord_hms_crm_record_count_17',
            label: 'CRM Record Count',
          }),
          search.createColumn({
            name: 'custrecord_hms_survery_count_17',
            label: 'Survey Count',
          }),
          search.createColumn({
            name: 'custrecord_hms_sold_properties_17',
            label: 'Sold Properties',
          }),
          search.createColumn({
            name: 'custrecord_hms_first_name_17',
            label: 'First Name',
          }),
          search.createColumn({
            name: 'custrecord_hms_last_name_17',
            label: 'Last Name',
          }),
          search.createColumn({
            name: 'custrecord_hms_last_update_17',
            label: 'Last Update',
          }),
        ],
      });
      ...
      if (searchResultCount == 2) {
        customrecord_hms_agent_upd_projectSearchObj.run().each((result) => {
          ...
          resultObj.agentIdNumber = result.getValue(
            'custrecord_hms_agent_id_number_17'
          );
          resultObj.verifiedFromRETSFeeds = result.getValue(
            'custrecord_hms_verified_from_ret_feed_17'
          );
          resultObj.cCount = result.getValue(
            'custrecord_hms_crm_record_count_17'
          );
          resultObj.sCount = result.getValue('custrecord_hms_survery_count_17');
          resultObj.spCount = result.getValue(
            'custrecord_hms_sold_properties_17'
          );
          resultObj.firstName = result.getValue('custrecord_hms_first_name_17');
          resultObj.lastName = result.getValue('custrecord_hms_last_name_17');
          resultObj.lastUpdate = result.getValue(
            'custrecord_hms_last_update_17'
          );
  ...
  const searchAgentDuplicateswithTwoRecords = () => {
    ...
    return search.create({
      ...
      columns: [
        search.createColumn({
          name: 'custrecord_hms_email_dupe_17',
          summary: 'GROUP',
          label: 'Email ID Duplicate',
        }),
        search.createColumn({
          name: 'custrecord_hms_agent_email_17',
          summary: 'GROUP',
          label: 'EMAIL',
        }),
        search.createColumn({
        ...
        search.createColumn({
          name: 'custrecord_hms_mls_region_17',
          summary: 'GROUP',
          label: 'MLS Region',
        }),
      ],
    ...
    try {
      if (customRecordId && flag == 'purge') {
        record.submitFields({
          ...
          id: customRecordId,
          values: {
            custrecord_hms_purge_17: true,
            custrecord_hms_keep_17: false,
          },
        });
        ...
        record.submitFields({
          ...
          id: customRecordId,
          values: {
            custrecord_hms_keep_17: true,
            custrecord_hms_purge_17: false,
          },
        });
        log.debug(loggerTitle, `Updated ID: ${customRecordId} for ${flag}`);
      }
