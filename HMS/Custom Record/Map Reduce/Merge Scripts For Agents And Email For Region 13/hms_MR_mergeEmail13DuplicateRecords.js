...
/**
 * File name: hms_MR_mergeEmail13DuplicateRecords.js
 * Script: HMS | MR Merge Email 13 Duplicate Records
...
define(['N/search', 'N/record', 'N/runtime'], (search, record, runtime) => {
  ...
  const reduce = (reduceContext) => {
    ...
    try {
      ...
      // Read the Values
      const email =
        reduceContextValues.values['GROUP(custrecord_hms_agent_email_13)'];
  ...
  const searchEmailDuplicateswithTwoRecords = () => {
    ...
    if (emailIdParameter) {
      filtersArr = [
        ...
        ['isinactive', 'is', 'F'],
        'AND',
        ['custrecord_hms_agent_email_13', 'is', emailIdParameter],
      ];
    } else {
      filtersArr = [
        ['custrecord_hms_email_dupe_13', 'is', 'T'],
