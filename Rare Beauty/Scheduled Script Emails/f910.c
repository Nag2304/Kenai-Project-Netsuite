...
/*@@@@@@@@@@@@@@@@@@@@@@@@@@@ END					*/
   ...
   ************************************************************************/
   DSD4000880 dsD4000880            = {0};   /* Verify And Get F0101 For OP */
...
/*@@@@@@@@@@@@@@@@@@@@@@@@@@@ END					*/
   ...
   */
   ...
   /************************************************************************
   * Determine if the Document Company exists in the UDC 56/TM
   ************************************************************************/
  
   /* Check Document Company */
   memset((void *)(&dsD0005),(int)(_J('\0')),sizeof(DSD0005));
   jdeStrcpy((JCHAR *)(dsD0005.szSystemCode),(const JCHAR *)(_J("56")));
   jdeStrcpy((JCHAR *)(dsD0005.szRecordTypeCode),(const JCHAR *)(_J("TM")));
   jdeStrcpy((JCHAR *)(dsD0005.szUserDefinedCode),(const JCHAR *)lpDS->szOrderCo);
   ParseNumericString(&dsD0005.mnKeyFieldLength,_J("2"));
   dsD0005.cSuppressErrorMessage =_J('1');

   idSoldShipReturnValue = jdeCallObject(_J("GetUDC"), NULL, lpBhvrCom,lpVoid,(LPVOID)&dsD0005,
                                       (CALLMAP *)NULL,(int)0,(JCHAR *)NULL,
                                       (JCHAR *)NULL,(int)0);
   if (idSoldShipReturnValue != ER_SUCCESS)
    {
      cSoldToOrShipToDefault = _J('1');
    } 
 ...
 * David Macek		14.06.2013				Replace D0100042 with D4200140		*/
        /* DWS.262.006.DD    Lee Balsom
         * At this point in the code, the Sold To Addr must contain a valid value.  
